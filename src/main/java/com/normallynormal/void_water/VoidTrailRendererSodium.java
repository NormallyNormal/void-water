        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            for (int depth = 1; depth <= trailLength; depth++) {
                ...
                if (renderFace || neighborTrail < depth) {
                    ...
                    // Sodium's translucent-moving-block pipeline horizontally mirrors the sampled
                    // texture vs. vanilla on 1.21.11, so we pre-flip U here (u1 at the west edge,
                    // u0 at the east edge) to cancel it out. Vanilla VoidTrailRenderer keeps the
                    // natural u0/u1 order since it doesn't go through Sodium's intercept.
                    vertex(buffer, pose, x1, yTop,    z1, r1,    g1,    b1,    topAlpha,    u1, v0, lt1,        nx, 0,  nz);
                    vertex(buffer, pose, x2, yTop,    z2, r2,    g2,    b2,    topAlpha,    u0, v0, lt2,        nx, 0,  nz);
                    vertex(buffer, pose, x2, yBottom, z2, flatR, flatG, flatB, bottomAlpha, u0, v1, flatLight,  nx, 0,  nz);
                    vertex(buffer, pose, x1, yBottom, z1, flatR, flatG, flatB, bottomAlpha, u1, v1, flatLight,  nx, 0,  nz);
